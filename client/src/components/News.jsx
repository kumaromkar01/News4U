...
const NewsPage = () => {
  ...
  const uniqueNews = allNews.filter((item, index, self) =>
    index === self.findIndex((t) => (
      t.postUrl === item.postUrl
    ))
  );

  // Sort news by date in descending order (latest first)
  uniqueNews.sort((a, b) => moment(b.date).unix() - moment(a.date).unix());

  const itemsPerPage = 12;
  const [currentPage, setCurrentPage] = useState(1);
  const totalPages = Math.ceil(uniqueNews.length / itemsPerPage);
  ...
  const currentItems = uniqueNews.slice(startIndex, endIndex);
